 ...
 * 
 * request.getRequestDispatcher()的各項操作: include()、forward()。
 * include()會回到原頁，而forward()會到指定的下一頁。
 * 
 * 值的傳遞部分: Attribute、Parameter。
 * Attribute是額外設定的屬性，僅在該次請求週期內有效；而Parameter指的是畫面上的表單值。操作方式例如:
 * request.setAttribute("dataAttribute", "default");    request.getAttribute("dataAttribute");
 * request.getParameter("name");
 * 
 * 在瀏覽器輸入http://localhost:8080/coreyou.servletAndJSP/someServlet.view測試。
 * 
 * @author coreyou
 */
@WebServlet(name="SomeServlet", urlPatterns={"/someServlet.view"})
public class SomeServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		out.println("<html>");
		out.println("<head>");
		out.println("<title>Some Servlet</title>");
		out.println("</head>");
		out.println("<body>");
